...
function embark(names) 
	...
	if mypos then --if mod already loaded
		...
		modpos=mypos
		_,modsize=engine.loadobj('plugins/embark/embark.o')
	...
	loc=offsets.find(stoff,0xa1,DWORD_,tofind,EOL)

	print("found:"..loc)

	if((loc~=0)and(loc-stoff<1000)) then
		modpos,modsize=engine.loadmod('plugins/embark/embark.o','Embark')
...
if not(FILE)then
	names=ParseNames("plugins/embark/races.txt")--io.open("plugins/embark/races.txt"):lines()
	embark(names)
end