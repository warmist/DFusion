...
if mypos then
	...
	modpos=mypos
	_,modsize=engine.loadobj("plugins/migrants/migrants.o")
	count=0
	...
	modpos,modsize=engine.loadmod("plugins/migrants/migrants.o","Migrants")
	...
	--patch part
	--pos=62873C+DF
	-- pattern: A1,DWORD_,"CURRENTRACE",56,89,ANYBYTE,ANYBYTE,34,e8,EOL
	pos=offsets.find(offsets.base(),0xa1,DWORD_,offsets.getEx("CurrentRace"),0x56,0x89,ANYBYTE,ANYBYTE,0x34,0xe8,EOL)
...
if not(FILE) then
	names=ParseNames("plugins/migrants/races.txt")--io.open("plugins/migrants/races.txt"):lines()
	migrants(names)
end