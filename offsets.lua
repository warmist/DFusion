...
function f_dwarves()
	pos=offsets.find(0,0x24,0x14,0x07,0,0,0,0xeb,0x08,0x8d,EOL) --search pattern
	if pos~=0 then
...
function f_creatureptr() --a creature number under the pointer
	pos=offsets.find(offsets.base(),0xa1,ANYDWORD,0x83,0xf8,0xff,0x75,EOL)
	if pos~=0 then
...
function f_racevec() --current race
	...
	local pos=0
	repeat
		pos=offsets.find(pos+3,0x0f,0xbf,ANYBYTE,ADDRESS,EOL)
...
function f_pointer() --adventure (not only?) pointer x,y,z
	...
	local pos=0
	repeat
		pos=offsets.find(pos+3,0x0f,0xb7,ANYBYTE,ADDRESS,EOL)
		local add=engine.peekd(pos+3)
...
function f_playerlegendid()
	...
	local pos=1
	repeat
		pos=offsets.find(pos+1,0xa1,DWORD_,off+4,0x2b,0x05,DWORD_,off,EOL)
...
function f_world()
	...
	T={}
	while pos~=0 do
		--pos=offsets.find(pos+6,0xa1,DWORD_,mapoffset,0x8b,0x4c,0x88,0xFC,EOL)
		pos=offsets.find(pos+6,0x8b,0x35,ANYDWORD,0x85,0xf6,EOL)--,0x8b,0x4c,0x88,0xFC,EOL)
		--pos2=offsets.find(pos,0x8b,0x34,0x88,EOL)
